#translates a number (0-127) to an array of seven bits, and the last
#bit is cut off so that BracketElements could be matched together based on
#identical bitsArrs
def position_to_six_bits(position):
    bitsArr = []
    for i in range(7):
        bit = position%2
        bitsArr = [bit]+bitsArr
...
bracket into an array of six bits. BracketElements with identical bitsArrs
...
class BracketElement:
    def __init__(self, position):
        self.bitsArr = position_to_six_bits(position)
        self.opponent = 'BYE'
        self.newPoints = 0
        self.lostTo = ''
